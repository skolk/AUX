...
void loop() {
  ...
  //dear heater, do not melt things
  monitorTemp();
}
...
// Get Paristalitc pump status 
  else if (cmdEqual(request, "PPST")) {
    bool peristalticPumpOn = getParistalticPumpOn();
    echoCommand(request);
    sendInt((int) peristalticPumpOn);
  }
...
else if (cmdEqual(request, "QUAD")) {
    char* requestParam = &request[3];
    uint8_t quadLocation = getUint8(requestParam);
//FIX
    //getQuad(quadLocation);
    echoCommand(request);
    //sendQuadLocation();
  }
